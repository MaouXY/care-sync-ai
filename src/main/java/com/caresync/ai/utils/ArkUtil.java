    ...
    public ChatContent botChat(ChatRequest request, String systemPrompt) {
        try {
            ...
            if (result != null && result.getChoices() != null && !result.getChoices().isEmpty()) {
                if (result.getChoices().get(0) != null && result.getChoices().get(0).getMessage() != null) {
                    Object contentObj = result.getChoices().get(0).getMessage().getContent();
                    if (contentObj != null) {
                        String content = contentObj.toString();
                        chatContent.setContent(content);
                    } else {
                        chatContent.setContent("AI服务未返回内容");
                    }
                    ...
                    chatContent.setContent("AI服务返回格式异常");
                }
            } else {
                chatContent.setContent("AI服务未返回有效响应");
            }

            // 如果有参考文献，可以添加到返回结果中
            if (result != null && result.getReferences() != null && !result.getReferences().isEmpty()) {
